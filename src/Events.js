...
class Events extends React.Component {
  ...
  editFilters = () => {
    const popup = (
      <Filters closePopup={this.closePopup} createAlert={this.createAlert} />
    );
    this.setState({ eventInfo: popup });
  };
  closePopup = () => {
  ...
  render() {
    const content = (
      <div className="page-wrapper events-wrapper">
        <h1>Events</h1>
        <Button clickHandler={this.createEvent} className="create-button">
          CREATE
        </Button>
        <Button clickHandler={this.editFilters} className="filters-button">
          FILTER
        </Button>
        <Clock />
        <div className="events-list">
          ...
          {this.state.nextPage && (
            <span
              className="events-list_pagenav"
              onClick={() => this.cyclePage(1)}
              style={{ top: 0, right: 0 }}
            >
              &gt;
            </span>
