...
class Login extends React.Component {
  ...
  handleSubmit = () => {
    ...
    fetch(localStorage.getItem("proxy") + "/users")
      .then((r) => r.json())
      .then((data) => {
  ...
  render() {
    const content = (
      <div className="page-wrapper">
        <div className="login-box">
          ...
          <br />
          <Button class="login-button" clickHandler={this.handleSubmit}>
            Login
          </Button>
        </div>
      </div>
    ...
    return (
      <App childClass="login">
        {content}
        {this.state.alert && (
          <Alert options={this.alertOptions} closePopup={this.closePopup}>
            <br />
            <Button clickHandler={this.signup}>Signup</Button>
          </Alert>
        )}
