...
export default class EventEdit extends Component {
  ...
  handleSubmit = () => {
    ...
    if (event.access === "Invite only") {
      event.whitelist = !this.state.event.whitelist
        ? []
        : this.state.event.whitelist.split(",").map((u) => u.trim());
    }
    if (
      !event.name ||
      !event.description ||
    ...
    if (event.cap.includes(".") || parseInt(event.cap) < 1) {
      return this.props.createAlert({
        type: -1,
        title:
          "Guest capacity must be an integer and greater than or equal to 1!",
      });
    }
  ...
  handleDelete = () => {
    fetch(
      localStorage.getItem("proxy") +
        "/events/delete?" +
        new URLSearchParams({ id: this.props.event.id }),
      {
        method: "DELETE",
      },
    ).then((r) => {
  ...
  render() {
    return (
      <div className="eventinfo-wrapper">
        <div
          className={
            "eventinfo-box" + (!this.state.map ? "" : " eventinfo-box_map")
          }
        >
              ...
              <>
                <div className="input__group">
                  ...
                  <input
                    type="text"
                    name="whitelist"
                    value={this.state.event.whitelist}
                    placeholder="username1, user2, ...(csv)"
                    onChange={this.handleInput}
                  />
                </div>
                <br />
          ...
          <Button
            className="eventedit_submit-btn"
            clickHandler={this.handleSubmit}
          >
            Submit
          </Button>
          {this.props.event === -1 ? null : (
            <Button
              className="eventedit_delete-btn"
