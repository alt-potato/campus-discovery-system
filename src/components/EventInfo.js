...
export default class EventInfo extends Component {
  constructor(props) {
    ...
    this.hasRSVP = -1;
    this.eventOwner =
      this.event.host.username === localStorage.getItem("user.username") ||
  ...
  updateRSVPs() {
    fetch(
      localStorage.getItem("proxy") + "/events/getRSVPs?id=" + this.event.id,
    ).then((r) => {
      if (r.status === 200) {
        r.json().then((data) => {
          this.hasRSVP = !data
            ...
            : data
                .map((r) => r.username)
                .indexOf(localStorage.getItem("user.username"));
          if (this.hasRSVP > -1) {
  ...
  handleRSVP = () => {
    ...
    if (
      this.event.access === "Invite only" &&
      localStorage.getItem("user.role") !== "Administrator"
    ) {
      if (
        this.event.whitelist &&
        !this.event.whitelist.includes(localStorage.getItem("user.username"))
      ) {
        return this.props.createAlert({
          type: -1,
          title:
            "This event is invite only! It is restricted to certain users and you are not invited/whitelisted by the host",
        });
      }
    }
    this.setState({
  ...
  deleteRSVP = (username) => {
    if (this.eventOwner && typeof username === "string") {
      ...
      fetch(localStorage.getItem("proxy") + "/events/deleteRSVP?" + params, {
        method: "DELETE",
      }).then((r) => {
        if (r.status === 200) {
          this.updateRSVPs();
  ...
  render() {
    ...
    if (this.state.RSVPs) {
      ...
      this.state.RSVPs.forEach((r) => {
        let user = (
          <p
            className={`rsvp-name${canDelete}`}
            key={r.username}
            onClick={(e) => this.deleteRSVP(r.username)}
          >
    ...
    return (
      <div className="eventinfo-wrapper">
        <div className="eventinfo-box">
          <p className="event_info event_name">{this.event.name}</p>
          ...
          <p className="event_info">
            Host:{" "}
            <span className="event_host">
              {this.event.host.name} ({this.event.host.username})
            </span>
          </p>
          <p className="event_info">
            Total RSVPs:{" "}
            <span className="event_numrsvps">
              {!this.state.RSVPs ? 0 : this.state.RSVPs.length}
            </span>
          </p>
          <p className="event_info">
          ...
          <Button
            className={`eventedit_${this.hasRSVP === -1 ? "submit" : "edit"}-btn`}
            clickHandler={this.handleRSVP}
          >
